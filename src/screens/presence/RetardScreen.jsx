 ...
 * @returns 
 */
export default function Retardscreen() {
    const user = useSelector(userSelector)
    const [allRetards, setRetards] = useState([])
    const [title, setTitle] = useState(user?.USERNAME)

    const [loading, setLoading] = useState(false)
    //fonction pour recuperer les retards  de l'utilisateur connecte
    useFocusEffect(useCallback(() => {
        (async () => {
            try {

                setLoading(true)
                const retards = await fetchApi(`/auth/users/retards`)
                setRetards(retards.result)
